...
People Playground has [its own mechanism](https://wiki.studiominus.nl/details/shadyCodeRejection.html)
for detecting dangerous mods. However, it only scans scripts and completely
ignores assemblies, marking all mods that use them as suspicious.
Since many advanced mods use them, they are marked as 
suspicious, even if there's no dangerous code inside.
AssScaner *aims* to fix this by scanning assemblies used
by mods and preventing potentially dangerous mods from loading.
